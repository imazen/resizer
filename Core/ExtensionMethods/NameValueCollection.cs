...
namespace ImageResizer.ExtensionMethods
{
    public static class NameValueCollectionExtensions
    {
        ...
        public static T? ParsePrimitive<T>(string value, T? defaultValue) where T : struct, IConvertible
        {
            ...
            {
                var val = EnumExtensions.Parse<T>(value); //Support EnumString values
        ...
        /// <summary>
        ///     Provides culture-invariant serialization of value types, in lower case for querystring readability. Setting a key
        ///     to null removes it.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="q"></param>
        ...
        /// <param name="val"></param>
        /// <returns></returns>
        public static NameValueCollection Set<T>(this NameValueCollection q, string name, T? val)
        ...
        public static T[] GetList<T>(this NameValueCollection q, string name, T? fallbackValue,
            params int[] allowedSizes) where T : struct, IConvertible
        {
            return ParseList<T>(q[name], fallbackValue, allowedSizes);
        }

        /// <summary>
        ///     Parses a comma-delimited list of primitive values. If there are unparsable items in the list, they will be replaced
        ///     with 'fallbackValue'. If fallbackValue is null, the function will return null
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        public static NameValueCollection SetList<T>(this NameValueCollection q, string name, T[] values,
            bool throwExceptions, params int[] allowedSizes) where T : struct, IConvertible
        {
            if (values == null)
            {
                q.Remove(name);
            ...
            if (!foundCount)
            {
                if (throwExceptions)
                    throw new ArgumentOutOfRangeException("values",
                        "The specified array is not a valid length. Valid lengths are " +
                        JoinPrimitives<int>(allowedSizes, ','));
                else return q;
            }
        ...
        /// <summary>
        ///     Creates and returns a new NameValueCollection instance that contains only the specified keys from the current
        ///     collection.
        /// </summary>
        /// <param name="q"></param>
        ...
        {
            var c = new NameValueCollection(q);
        ...
        public static NameValueCollection MergeDefaults(this NameValueCollection q, NameValueCollection defaults)
        {
            ...
            var c = new NameValueCollection(defaults);
            foreach (var s in q.AllKeys) c[s] = q[s];
