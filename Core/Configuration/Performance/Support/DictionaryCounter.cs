...
{
    internal class DictionaryCounter<T>
    {
        private readonly ConcurrentDictionary<T, Counter> dict;
        private readonly Counter count;
        private readonly Counter otherCount;
        ...
        public DictionaryCounter(int maxKeyCount, T limitExceededKey) : this(maxKeyCount, limitExceededKey,
        ...
        public DictionaryCounter(int maxKeyCount, T limitExceededKey, IEnumerable<KeyValuePair<T, long>> initial,
            ...
            for (var retryCount = 0; retryCount < 10; retryCount++)
            {
                ...
                else
                {
                    if (!applyLimitSwap)
                    {
                        ...
                        if (result != newValue) count.Decrement();
                        return result;
                    }
                    else
                    {
