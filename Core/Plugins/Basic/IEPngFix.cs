...
namespace ImageResizer.Plugins.Basic
{
    ...
    public class IEPngFix : IPlugin, IQuerystringPlugin
    {
        ...
        /// <summary>
        ///     If true, the requests from IE will be HTTP redirected to new URLs. If false, the GIF will be silently served
        ///     instead of the PNG, without any redirection.
        ///     A CDN or caching proxy will mess things up regardless, but using redirection ensures that the CDN/proxy never
        ///     caches the GIF version instead of the PNG.
        /// </summary>
        ...
        private void Pipeline_RewriteDefaults(IHttpModule sender, HttpContext context, IUrlEventArgs e)
        {
            ...
            if (NeedsPngFix(context) && CatchAll)
                e.QueryString["iefix"] =
                    "true"; //If CatchAll is enabled, it's a png, and we set the default value here.
        }
        ...
        private void Pipeline_PostRewrite(IHttpModule sender, HttpContext context, IUrlEventArgs e)
        {
            if (e.QueryString.Get("iefix", false) && NeedsPngFix(context) && DestFormatPng(e))
            {
                if (Redirect)
                {
                    ...
                    context.Response.Redirect(PathUtils.MergeOverwriteQueryString(context.Request.RawUrl, newValues),
                        true);
                }
                else
                {
        ...
        {
            var guessedEncoder = c.Plugins.GetEncoder(new ResizeSettings(e.QueryString), e.VirtualPath);
            return guessedEncoder != null &&
                   "image/png".Equals(guessedEncoder.MimeType, StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static Regex ie456 = new Regex("\\sMSIE\\s[123456]\\.[0-9a-zA-Z]+;\\sWindows",
            RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase);

        /// <summary>
        ///     Returns true if the user agent string specifies MSIE versions 1 through 6 for Windows. Cached in context.Items
        ///     after first call.
        /// </summary>
        ...
        public static bool NeedsPngFix(HttpContext context)
        {
            ...
            {
                var needsFix = ie456.IsMatch(context.Request.UserAgent);
