...
*/
        ...
        /// <returns></returns>
        public static RectangleF AlignWith(RectangleF obj, RectangleF container, AnchorLocation align) {
            if (align == AnchorLocation.BottomLeft || align == AnchorLocation.MiddleLeft || align == AnchorLocation.TopLeft)
                obj.X = container.X;
            if (align == AnchorLocation.BottomCenter || align == AnchorLocation.MiddleCenter || align == AnchorLocation.TopCenter)
                obj.X = container.X + (container.Width - obj.Width) / 2;
            if (align == AnchorLocation.BottomRight || align == AnchorLocation.MiddleRight || align == AnchorLocation.TopRight)
                obj.X = container.X + (container.Width - obj.Width);
            if (align == AnchorLocation.TopLeft || align == AnchorLocation.TopCenter || align == AnchorLocation.TopRight)
                obj.Y = container.Y;
            if (align == AnchorLocation.MiddleLeft || align == AnchorLocation.MiddleCenter || align == AnchorLocation.MiddleRight)
                obj.Y = container.Y + (container.Height - obj.Height) / 2;
            if (align == AnchorLocation.BottomLeft || align == AnchorLocation.BottomCenter || align == AnchorLocation.BottomRight)
        ...
        /// <param name="align"></param>
        /// <returns></returns>
        public static PointF[] AlignWith(PointF[] obj, PointF[] container, AnchorLocation align) {
        ...
        /// <returns></returns>
        public static FlipMode CombineFlipAndRotate(FlipMode flip, double angle) {
            angle = NormalizeTo90Intervals(angle);
            if (flip == FlipMode.None) {
                return (FlipMode)(int)(angle / 90);
            } else if (flip == FlipMode.X) {
                return (FlipMode)(int)(4 + (angle / 90));
            } else if (flip == FlipMode.Y) {
                if (angle == 0) return (FlipMode)6;
                if (angle == 90) return (FlipMode)7;
                if (angle == 180) return (FlipMode)4;
                if (angle == 270) return (FlipMode)5;
            } else if (flip == FlipMode.XY) {
                if (angle == 0) return (FlipMode)2;
                if (angle == 90) return (FlipMode)3;
                if (angle == 180) return (FlipMode)0;
                if (angle == 270) return (FlipMode)1;
            } else {
