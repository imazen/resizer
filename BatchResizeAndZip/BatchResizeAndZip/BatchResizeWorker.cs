...
namespace fbs.ImageResizer.BatchZipper
{
    ...
    public class BatchResizeWorker
    {
        ...
        {
            //Can we resize it?
            bool resize = ImageOutputSettings.IsAcceptedImageType(System.IO.Path.GetFileName(i.PhysicalPath));
        ...
        protected void WriteItemCallback(string entryName, System.IO.Stream stream)
        {
            ...
            if (wouldResize(i))
            {
                ...
                NameValueCollection queryString = new yrl(i.ResizeQuerystring).QueryString;
                //Resize the image based on the querystring
                using (System.Drawing.Bitmap resized = fbs.ImageResizer.ImageManager.getBestInstance().BuildImage(i.PhysicalPath, queryString))
                {
                    //Determines output format, includes code for saving in a variety of formats.
                    ImageOutputSettings ios = new ImageOutputSettings(ImageOutputSettings.GetImageFormatFromPhysicalPath(i.PhysicalPath), queryString);
                    //Save, assuming we can't seek within the zip output stream.
                    ios.SaveImageToNonSeekableStream(stream, resized);
                }
                return; //We're done!
