    ...
    public class BatchResizeSettings
    {
        ...
        internal void AppendFinalExtensions()
        {
            foreach (BatchResizeItem i in files)
            {
                ...
                //Can we resize it?
                if (fbs.ImageResizer.ImageOutputSettings.IsAcceptedImageType(originalName))
                {
                    //Add the correct (possibly changed) file extension.
                    i.TargetFilename += "." + new fbs.ImageResizer.ImageOutputSettings(new yrl(originalName + i.ResizeQuerystring)).GetFinalExtension();
                }
                else
                {
