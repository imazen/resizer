...
namespace ImageResizer.Resizing
{
    ...
    public enum RequestedAction
    {
        ...
        /// <summary>
        ///     Requests that ImageBuilder cancels the default logic of the method, and stop executing plugin calls for the method
        ///     immediately.
        /// </summary>
        Cancel
    }
    ...
    ///     Each method of AbstractImageProcessor loops through all extensions and executes the same method on each. Provides a
    ///     sort of multiple-inheritance mechanism.
    /// </summary>
...
#pragma warning restore CS0618
        ...
        /// </summary>
        [CLSCompliant(false)] protected volatile IEnumerable<BuilderExtension> exts;
        ...
        protected virtual void PreLoadImage(ref object source, ref string path, ref bool disposeSource,
            ref ResizeSettings settings)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    p.PreLoadImage(ref source, ref path, ref disposeSource, ref settings);
        }

        /// <summary>
        ///     Extend this to allow  additional types of source objects to be accepted by transforming them into Stream instances.
        ///     First plugin to return a Stream wins.
        /// </summary>
        /// <param name="source"></param>
        /// <param name="settings"></param>
        ...
        /// <returns></returns>
        protected virtual Stream GetStream(object source, ResizeSettings settings, ref bool disposeStream,
            out string path, out bool restoreStreamPosition)
        {
            ...
            if (exts != null)
                ...
                {
                    var disposeS =
                        disposeStream; //Copy the referenced boolean. Only allow plugins who return a stream to change its value
                    var s = p.GetStream(source, settings, ref disposeS, out path, out restoreStreamPosition);
                    if (s != null)
                    {
        ...
        ///     If extensions throw an ArgumentException or ExternalException, it will be wrapped in an ImageCorruptedException
        ///     instance.
        ///     If the Bitmap class is used for decoding, read gdi-bugs.txt and make sure you set b.Tag to new
        ///     BitmapTag(optionalPath,stream);
        /// </summary>
        public virtual Bitmap DecodeStreamFailed(Stream s, ResizeSettings settings, string optionalPath)
        {
            ...
            foreach (AbstractImageProcessor p in exts)
            {
                ...
                var b = p.DecodeStreamFailed(s, settings, optionalPath);
                if (b != null) return b;
            }
        ...
        public virtual Bitmap DecodeStream(Stream s, ResizeSettings settings, string optionalPath)
        {
            ...
            foreach (AbstractImageProcessor p in exts)
            {
                var b = p.DecodeStream(s, settings, optionalPath);
                if (b != null) return b;
            }
        ...
        protected virtual RequestedAction PostDecodeStream(ref Bitmap img, ResizeSettings settings)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.PostDecodeStream(ref img, settings) == RequestedAction.Cancel)
        ...
        protected virtual void PreAcquireStream(ref object dest, ResizeSettings settings)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    p.PreAcquireStream(ref dest, settings);
        }
        ...
        protected virtual RequestedAction InternalGraphicsDrawImage(ImageState state, Bitmap dest, Bitmap source,
            PointF[] targetArea, RectangleF sourceArea, float[][] colorMatrix)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.InternalGraphicsDrawImage(state, dest, source, targetArea, sourceArea, colorMatrix) ==
        ...
        ///     Only override this method if you need to replace the behavior of image encoding and image processing together, such
        ///     as adding support
        ...
        /// <summary>
        ///     Process.0 First step of the Process() method. Can replace the entire Process method if RequestAction.Cancel is
        ///     returned.
        ...
        protected virtual RequestedAction OnProcess(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.OnProcess(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        protected virtual RequestedAction PrepareSourceBitmap(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.PrepareSourceBitmap(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        protected virtual RequestedAction PostPrepareSourceBitmap(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.PostPrepareSourceBitmap(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        protected virtual RequestedAction Layout(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.Layout(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }

        /// <summary>
        ///     Process.3(Layout).1: This is where the points in the layout are flipped the same way the source bitmap was flipped
        ///     (unless their flags specify otherwise)
        /// </summary>
        /// <param name="s"></param>
        protected virtual RequestedAction FlipExistingPoints(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.FlipExistingPoints(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        protected virtual RequestedAction LayoutImage(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.LayoutImage(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        protected virtual RequestedAction PostLayoutImage(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.PostLayoutImage(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        protected virtual RequestedAction LayoutPadding(ImageState s)
        {
            if (exts != null)
                foreach (AbstractImageProcessor p in exts)
                    if (p.LayoutPadding(s) == RequestedAction.Cancel)
                        return RequestedAction.Cancel;
            return RequestedAction.None;
        }
        ...
        /// <summary>
        ///     Process.3(Layout).anytime: Occurs when the layout point values are rounded to integers. May be called anytime
        ///     during Layout()
        /// </summary>
        /// <param name="s"></param>
        ...
        /// <summary>
        ///     Process.3(Layout).anytime: Occurs after the layout point values are rounded to integers. May be called anytime
        ///     during Layout()
        /// </summary>
        /// <param name="s"></param>
        ...
        /// <summary>
        ///     Process.3(Layout).12: Occurs once layout has finished. No more changes should occur to points or rings in the
        ///     layout after this method. destSize is calculated here.
        /// </summary>
        /// <param name="s"></param>
        ...
        /// <summary>
        ///     Process.4: The destination bitmap is created and sized based destSize. A graphics object is initialized for
        ///     rendering.
        /// </summary>
        /// <param name="s"></param>
        ...
        /// <summary>
        ///     Process.5(Render) Rendering. Do not return RequestedAction.Cancel unless  you want to replace the entire rendering
        ///     system.
        /// </summary>
        /// <param name="s"></param>
        ...
        /// <summary>
        ///     Process.5(Render).9: Plugins have a chance to pre-process the source image before it gets rendered, and save it to
        ///     s.preRenderBitmap
        /// </summary>
        /// <param name="s"></param>
        ...
        /// <summary>
        ///     Process.6: Non-rendering changes to the bitmap object occur here, such as flipping. The graphics object is
        ///     unavailable.
        /// </summary>
        /// <param name="s"></param>
