...
namespace ImageResizer.Util
{
    ...
    public class PathUtils
    {
        ...
        /// </summary>
        public static string AppVirtualPath => HostingEnvironment.ApplicationVirtualPath != null
        ...
        /// </summary>
        public static string AppPhysicalPath => HostingEnvironment.ApplicationPhysicalPath != null
        ...
        {
            var query = path.IndexOfAny(QueryOrFragment);
            if (query < 0) query = path.Length;
            //Finds the first character that could possibly be part of the extension (before the query)
            var firstPossibleExtensionChar = path.LastIndexOfAny(SpaceOrSlash, query - 1) + 1;
            var extensionStarts = path.IndexOf('.', firstPossibleExtensionChar, query - firstPossibleExtensionChar);
            if (extensionStarts < 0) extensionStarts = query;

            return path.Substring(0, extensionStarts) +
        ...
        /// <summary>
        ///     Removes the extension from the filename or URL, leaving the querystring intact, where the extension is only the
        ///     last extension segment.
        ...
        public static string RemoveExtension(string path)
        {
            ...
            //Finds the first character that could possibly be part of the extension (before the query)
            var firstPossibleExtensionChar = path.LastIndexOfAny(SpaceOrSlash, query - 1) + 1;
            var extensionStarts = path.LastIndexOf('.', query - 1, query - firstPossibleExtensionChar);
            if (extensionStarts < 0) extensionStarts = query;
        ...
        /// <summary>
        ///     Grabs the last segment of the filename extension. Returns an empty string if there is no extension, or if the
        ...
        {
            var applicationVirtualPath = HostingEnvironment.ApplicationVirtualPath ?? string.Empty;
        ...
        /// <summary>
        ///     Joins the path and querystring. If the path already contains a querystring, they are 'append joined' with the
        ///     correct character. Fragment is maintained as-is.
        ...
        public static string AddQueryString(string virtualPath, string querystring)
        {
            ...
            var fragment = virtualPath.IndexOf('#');
            var suffix = "";
            if (fragment < 0)
            {
                fragment = virtualPath.Length;
            }
            else
            {
        ...
        /// <summary>
        ///     Removes the query string from the specified path. If the path is only a querystring, an empty string is returned.
        ///     Does not support the semicolon syntax. Fragment is maintained as-is.
        /// </summary>
        /// <param name="path"></param>
        /// <returns></returns>
        ...
        /// <summary>
        ///     Overwrites existing querystring values in 'path' with the values in 'newQuerystring'. Does not support the
        ///     semicolon syntax.
        /// </summary>
        /// <param name="path"></param>
        ...
        {
            var oldQuery = ParseQueryString(path);
        ...
        /// <summary>
        ///     Adds the querystring values in 'newQuerystring' to the querystring in Path, but does not overwrite values. Does not
        ///     support the semicolon syntax.
        /// </summary>
        /// <param name="path"></param>
        /// <param name="newQuerystring"></param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        public static string BuildQueryString(NameValueCollection QueryString, bool urlEncode,
            bool skipNullValues = true, char firstSeparator = '?', char laterSeparators = '&', char equals = '=')
        {
        ...
        ///     Parses the querystring from the given path into a NameValueCollection.
        ///     accepts "file?key=value" and "?key=value&amp;key2=value2" formats. (no path is required)
        ///     UrlDecodes keys and values. Does not enforce correct syntax, I.E. '?key=value?key2=value2' is allowed. However, '
        ///     &amp;key=value?key2=value' will only get key2 parsed.
        ///     When allowSemicolons is true, semicolon paths like ';key=value;key2=value2' are allowed, as are hybrid paths:
        ...
        /// <param name="path"></param>
        /// <param name="allowSemicolons"></param>
        /// <param name="beforeQuery">
        ///     Returns the portion of the 'path' before the querystring. May include the scheme, server,
        ///     port, path and path info, depending upon what 'path' contained.
        /// </param>
            ...
            out string fragment)
        {
            ...
            if (allowSemicolons)
            {
                ...
                var firstsemicolon = path.IndexOf(';');
                if (firstdelimiter < 0 || (firstsemicolon >= 0 && firstsemicolon < firstdelimiter))
                    firstdelimiter = firstsemicolon;
            }
        ...
        /// <summary>
        ///     Parses a querystring into a name/value collection. The given string cannot include path or fragment information -
        ///     it must be *just* the querystring.
        /// </summary>
            ...
            bool urlDecode = true)
        {
            ...
            foreach (var s in pairs)
            {
                ...
                if (namevalue.Length > 2)
                {
                    ...
                    var value = s.Substring(s.IndexOf('=')).TrimStart('=');
        ...
        public static byte[] FromBase64UToBytes(string data)
        {
            data = data.PadRight(data.Length + (4 - data.Length % 4) % 4,
                '='); //if there is 1 leftover octet, add ==, if 2, add =. 3 octets = 4 chars.
        ...
        {
            return ToBase64U(System.Text.Encoding.UTF8.GetBytes(data));
        }
        ...
        {
            return System.Text.Encoding.UTF8.GetString(FromBase64UToBytes(data));
        }

        /// <summary>
        ///     Returns the physical mapped path for the specified virtual path if it starts with ~, otherwise returns the original
        ///     path.
        /// </summary>
        /// <param name="path"></param>
        /// <returns></returns>
        ...
        /// <summary>
        ///     Attempts to guess the virtual path from physical path. Will be thwarted if the path is mapped as a virtual folder
        ...
        ///     Standard variables are &lt;ext> (the default extension for the final file type), &lt;guid>, a randomly generated
        ///     GUID,
        ///     &lt;filename>, the original filename without it's extension, &lt;path>, the original path and filename without
        ///     extension,
        ///     &lt;settings.width>, (any specified settings value except preset), &lt;width> (final width), and &lt;height> (final
        ///     height).
        /// </summary>
        ...
        public static string ResolveVariablesInPath(string pathWithVars, VariableResolverCallback resolver)
        {
            ...
            {
                var start = p.IndexOf('<');
                var stop = p.IndexOf('>', start);
                var bugcheck = p.IndexOf('<', start + 1);
                if (stop < 0 || (bugcheck > -1 && bugcheck < stop) || stop == start + 1)
                    throw new ImageProcessingException(
                        "Destination paths can only contain < and > in matched pairs to designate variables. Path \"" +
                        pathWithVars + "\" has invalid syntax");
                var varName = p.Substring(start + 1, stop - start - 1);
                string filter = null;
                //Split off the filter if present.
                var filterIx = varName.IndexOf(':');
                if (filterIx > 0)
                {
                ...
                var result = resolver(varName);
                if (!string.IsNullOrEmpty(filter)) result = RemoveNonMatchingChars(result, filter);
                if (result == null)
                    throw new ImageProcessingException("Invalid variable name \"" + varName +
                                                       "\" in templated path \"" + pathWithVars +
                                                       "\". The variable name may be misspelled, or the variable may not be available with the pipeline you are using.");
        ...
        public static string RemoveNonMatchingChars(string text, string chars)
        {
            ...
            var r = new Regex("[" + Regex.Escape(chars) + "]", RegexOptions.Compiled);
            for (var i = 0; i < text.Length; i++)
            {
                var chr = text.Substring(i, 1);
