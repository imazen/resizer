...
namespace ImageResizer.Plugins.ImageflowPlugin
{
    public class ImageflowBuilderPlugin : BuilderExtension, IPlugin, IIssueProvider, IFileExtensionPlugin
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(builderString) && _defaultBuilder)
            {
                return true;
            }
        ...
        private void ResolvePathAndCreateDir(string templatePath, ImageJob job, BuildDecodeResult firstDecodeResult, BuildEncodeResult result)
        {
        ...
        private void DoJobWithImageflow(Stream source, ImageJob job)
        {
            ...
            using (var imageflowJob = new Imageflow.Fluent.ImageJob())
            {
                ...
                if (encodeBytesObject?.Array == null) throw new InvalidOperationException("Encoded bytes not available");
                ...
                switch (job.Dest)
                {
                    case string dest:
                    {
                        ...
                        using (var fs = new FileStream(job.FinalPath, FileMode.OpenOrCreate, FileAccess.Write,
                                   FileShare.None))
                        {
                            fs.Write(encodeBytes.Array ?? throw new InvalidOperationException(), encodeBytes.Offset, encodeBytes.Count);
                        }
                    ...
                    case Stream dest:
                    {
                        ...
                        outputStream.Write(encodeBytes.Array ?? throw new InvalidOperationException(), encodeBytes.Offset, encodeBytes.Count);
                        break;
                    }
                    default:
